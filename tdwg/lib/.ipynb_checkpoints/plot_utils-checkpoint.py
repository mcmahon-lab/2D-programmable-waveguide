...
def plot_norm_2col(xs1_list, xs2_list, ys1_list, ys2_list, args_list=None, kwargs_list=None, title_list=None, col_width=5, row_height=1, suptitles=None, norm_flag=True):
    """
    TODO
    Example use case (Put here later)
    """
...
def autoscale_vmin_vmax(ax=None, xaxis=None, yaxis=None):
    ...
    xaxis and yaxis can be input in an optional manner. 
    Note that for imshow the code is fast so this is not necesary. 
    For pcolormesh, you might want to input xaxis and yaxis since it can take about 2 seconds (not a disaster, but still kind of slow)
    """
    if ax is None:
        ax = plt.gca()
