...
TODO
    - Add a beamshaper sleep entry to calibration_dict
"""
...
def init_worker():
    """
    Essentially provide all of the functions needed so it can run fast!
    """
...
class TDwgExperiment():
    ...
    def update_cam_background(self, lock_flag=True):
        """
        The lock_flag when set to true, can ironically unlock something else, so be careful when using this function!
        """
        if lock_flag:
            self.client.lock()
    ...
    def run_exp_fixed_DMD_expcoord(self, Ein_bs: np.ndarray) -> np.ndarray:
        """
        This is the "deepest" function that provides the most basic functionality...
        """
    ...
    def run_exp_fixed_DMD(self, Ein_sim: torch.Tensor) -> torch.Tensor:
        """
        This function normalizes the output to have the same power as the input
        Also takes care of all of the conversions that are necessary to take in stuff from sim coordinates. (the lack of expcoord makes it implicit)
        Note that locking must be taken care of from the outside when using this function.
        Also takes care of amp_cali... man, this function does way too much, without "exposing" this publically...
        """
    ...
    def run_exp(self, img, Ein_sim: torch.Tensor) -> torch.Tensor:
        """
        Also used in a variety of settings! When just want to test with 1 image and all
        """
    ...
    def run_exp_Ein_list(self, img, Ein_sim_list: torch.Tensor, fast_flag=True) -> torch.Tensor:
        """
        This is implicitly in the coordinate of the sim. 
        The function does 3 things!
        1. It normalizes the output, so it has the same power as the input
        2. Takes in tensors and returns tensors.
        3. Also takes care of the locking.
        4. It also does an update of the camera background! This is a huge suprise, added by Martin, so I am keeping it. 
        """
        with self.client.locked():
            self.dmd.apply_image_hold(img, bitDepth=8)
            time.sleep(self.DMD_sleep) #This also should be a parameter ideally.
    
            # self.update_cam_background(lock_flag=False) #wow, the fact that this is done in the loop is a huge suprise...
            if fast_flag:
        ...
        # Iout_x_sim = torch.from_numpy(np.vstack(out_np))
    ...
    def create_pool(self):
        #first do a quick check about mode before deciding to create!
...
#         self.modulation_efficiency = nn.Parameter(torch.tensor(1.), requires_grad = train_flag)
    ...
    def run_sim_fixed_DMD(self, Ein_sim: torch.Tensor, fast_flag=True):
        ...
        """applying dn_norm_func mainly rescales the data but this makes a huge difference
        in the learning speed. This is an important reminder that all the physical parameters
        should ideally be set to be order 1."""
        self.background_delta_n = self.converter.dn_norm_func(self.background_mod)
    ...
    def run_sim_from_mod_sim_window(self, Ein_sim: torch.Tensor, mod_sim_window, fast_flag=True):
        """
        Basically here, despite "self.forward" being a function, because this one can set the fast_flag, which might mess with the f_pat thingy.
        """
...
class TDwgLayer(nn.Module):
    """
    Replaced previous code, by calling this "layer". 
    By layer, I mean that this object handles the input_mode and 
        output_mode transformation as well.
    """
